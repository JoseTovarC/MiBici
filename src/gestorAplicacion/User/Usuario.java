...
public class Usuario extends Persona {
	...
	public Usuario(String nombre, String edad, String iden, String genero, String clave, String sal, String ref) { // Constructor
																													// moderador
		super(nombre, edad, iden, genero, clave);
		int saldo = Integer.parseInt(sal);
		this.tarjeta = new Tarjeta(saldo, this);
	...
	public boolean isDeuda() { // se chequea si hay una deuda (si es que hay multas), cambia el atributo
								// "deuda" y ademas retorna true si se tienen multas o false en caso contrario
		if (multas.size() == 0) {
			this.deuda = false;
		} else if (multas == null) {
			this.deuda = false;
	...
	public void setDeuda(boolean deuda) {
		this.deuda = deuda;
	} // se puede modificar la deuda manualmente

	public long getDeuda() { // se devuelve el precio de la deuda actual (todas las multas sumadas) y se
								// cambia el atributo booleano "deuda"
		long deuda = 0;
		for (Multa m : multas) {
			deuda += m.getPrecio();
		}
...
//Al final, se llama a la estacion para que haga el prestamo de la bicicleta, se anexa la bicicleta al usuario, se le disminuye el saldo a su tarjeta por el prestamo y se devuelve un mensaje diciendo los datos de la persona que ha prestado la bicicleta y los datos de la bicicleta
		...
		if ((this.bicicleta == null) && !this.isDeuda()) {
			if (estacion.getCantBicis() > 0 && tarjeta.getSaldo() >= 500 && estacion.isEstado()) {
				...
				System.out.print("Elija una posici�n valida en la que halla una bicicleta: ");
				int idB = ent.nextInt() - 1;
				while (idB < 0 || idB > estacion.getCap_max()) {
					System.out.println("Por favor ingrese un valor valido. Entre 1 y " + (estacion.getCap_max()) + ".");
					idB = ent.nextInt() - 1;
				}
				...
				r.append("\n" + super.nombre + " posee la bicicleta " + this.bicicleta
						.toString() /* +". Estaba en la estacion " + estacion.getId()+" posicion "+idB */);
				tarjeta.pagarP();
	...
	public boolean prestarPanel(Estacion estacion, int idB) {
		// Se le presta una bicicleta al usuario, primero se ingresa una estacion y se
		// chequea todo (que no tenga un prestamo ni deudas actualmente, que la estacion
		// tenga bicicletas y esté abierta, que el usuario tenga saldo suficiente para
		// prestar una bicicleta)
		// Luego se imprime por pantalla las bicicletas que se encuentran en dicha
		// estacion y se le pide al usuario que ingrese un numero para elegir la
		// bicicleta
		// Despues, se confirma que este numero sea un valor valido para prestar una
		// bicicleta.
		// Al final, se llama a la estacion para que haga el prestamo de la bicicleta,
		// se anexa la bicicleta al usuario, se le disminuye el saldo a su tarjeta por
		// el prestamo y se devuelve un mensaje diciendo los datos de la persona que ha
		// prestado la bicicleta y los datos de la bicicleta
		if ((this.bicicleta == null) && !this.isDeuda() && estacion.getCantBicis() > 0 && tarjeta.getSaldo() >= 500
				&& estacion.isEstado()) {
	...
	public boolean devolver(Estacion estacion/* , Date initialtime */) {
...
//En caso contrario, si todo esta en orden se le notifica al usuario, se hace la devolucion a la estacion y se desconecta del usuario. 
	...
	public String pagarM() { // El usuario paga todas las multas, se revisa si tiene saldo suficiente en su
								// tarjeta y se devuelve el correspondiente mensaje
		if (this.isDeuda()) {
			if (tarjeta.pagarM()) {
	...
	public String pagarM(String id) { // El usuario paga una multa, se revisa si tiene saldo suficiente en su tarjeta
										// y se devuelve el correspondiente mensaje
		if (this.isDeuda()) {
